...
io.on('connection', (socket) => {
  ...
  socket.on('forfeit', () => {
    ...
    room.gameOver = true;
    room.winner = p === 0 ? 2 : 1;
    socket.emit('state', { ...getPublicState(room), forfeit: true });
    socket.to(socket.roomCode).emit('state', { ...getPublicState(room), opponentForfeited: true });
  });
  ...
  socket.on('move', ({ fromRow, fromCol, toRow, toCol }) => {
    ...
    const cols = 'abcdefgh';
    const fromNotation = `${cols[fromCol]}${8 - fromRow}`;
    const toNotation   = `${cols[toCol]}${8 - toRow}`;
    const moveEntry = {
      player: p + 1,
      piece: piece.type,
      from: fromNotation,

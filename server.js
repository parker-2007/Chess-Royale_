...
function aiTick(roomCode) {
  ...
  const AI = 1;   // AI is player index 1 (player 2, top of board)
  const elixir = room.elixir[AI];
  const now = Date.now();

  // Collect all AI pieces and their valid moves
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      ...
      for (const { row, col } of moves) {
        ...
        // Prioritize capturing king = instant win
        if (target?.type === 'king') score += 100000;

        // Advance toward enemy king (player 1 king is at bottom)
        ...
        // Prefer cheaper moves (save elixir for later)
        score -= cost * 0.5;

        // Small random noise so AI doesn't feel robotic
        score += Math.random() * 2;

        if (score > bestScore) { bestScore = score; bestMove = { fromRow:r, fromCol:c, toRow:row, toCol:col, cost, piece }; }
      }
    }
  }

  // Try deploying a card if it scores well
  ...
  for (let i = 0; i < hand.length; i++) {
    ...
    // Find an empty spot on deploy row
    const emptyCols = [];
    for (let c = 0; c < 8; c++) if (!room.board[deployRow][c]) emptyCols.push(c);
    if (emptyCols.length === 0) continue;

    // Pick col closest to enemy king
    let p1KingCol = 4;
    for (let rr = 0; rr < 8; rr++) for (let cc = 0; cc < 8; cc++)
    ...
    const deployCol = emptyCols[0];

    let deployScore = PIECE_VALUE[type] * 10 - cost * 0.5 + Math.random() * 2;
    // Only deploy if we have enough elixir to spare
    if (elixir > cost + 3 && deployScore > bestScore) {
      bestScore = deployScore;
      bestMove = { deploy: true, cardIdx: i, row: deployRow, col: deployCol, cost };
    }
  }
...
function startAITicker(roomCode) {
  ...
  // AI acts every 1.5-2.5 seconds (randomized so it feels natural)
...
io.on('connection', (socket) => {
  ...
  socket.on('playAI', () => {
    ...
    console.log(`AI game started for ${socket.id}`);
  });

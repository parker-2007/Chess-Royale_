...
// ── Spells ─────────────────────────────────────────────────────────────────
const SPELL_DATA = {
  freeze: { name:'Freeze', icon:'❄️', cost:3, area:3, description:'3x3 — Adds 5s cooldown to all pieces' },
  mud:    { name:'Mud',    icon:'🟤', cost:3, area:2, description:'2x2 — Doubles move cost for 10s' },
  haste:  { name:'Haste',  icon:'⚡', cost:7, area:1, description:'1x1 — Removes cooldown instantly' },
};
const SPELL_TYPES = ['freeze','mud','haste'];

function buildQueue() {
  // Equal chance: interleave one spell for every one piece card
  const pieces=shuffle([...PIECE_TYPES,...PIECE_TYPES,...PIECE_TYPES]);
  const spells=shuffle([...SPELL_TYPES,...SPELL_TYPES,...SPELL_TYPES,
                        ...SPELL_TYPES,...SPELL_TYPES,...SPELL_TYPES,
                        ...SPELL_TYPES,...SPELL_TYPES,...SPELL_TYPES,
                        ...SPELL_TYPES]);
  // Zip them together alternating piece/spell
  const result=[];
  const len=Math.max(pieces.length,spells.length);
  for(let i=0;i<len;i++){
    if(i<pieces.length) result.push(pieces[i]);
    if(i<spells.length) result.push(spells[i]);
  }
  return result;
}
const AI_SETTINGS = {
...
function createRoom(roomCode) {
  rooms[roomCode] = {
    ...
    deployRows:[[7],[0]],
    kills:[0,0],
    effects:[], // active spell effects: {type, squares:[{r,c}], expiry, caster}
  };

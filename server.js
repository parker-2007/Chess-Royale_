...
function buildCycleFromDeck(deck) {
  // CR style: shuffle the deck ONCE, then repeat that exact same order
  const order = shuffle([...deck]);
  const cycle = [];
  for(let i=0;i<20;i++) cycle.push(...order); // 20 repetitions = plenty
  return cycle;
}
const AI_SETTINGS = {
...
function dealHand(room,idx) {
  while(room.hands[idx].length<3){
    if(room.queueIdx[idx]>=room.cycles[idx].length){
      // Extend by repeating the same order (first 6-card rotation)
      const deckSize=room.decks[idx].length;
      const oneRotation=room.cycles[idx].slice(0,deckSize);
      room.cycles[idx].push(...oneRotation,...oneRotation,...oneRotation,...oneRotation,...oneRotation);
    }
...
function getMoveCostWithEffects(room, type, fr, fc, tr, tc, movingPlayer) {
  ...
  const inMud=room.effects.some(e=>
    ...
    e.caster!==movingPlayer && // only slows the enemy
...
function castSpell(room, spellType, centerRow, centerCol, casterPlayer) {
  ...
  if(spellType==='freeze'){
    // Add 5s cooldown to enemy pieces only
    squares.forEach(({r,c})=>{
    ...
    // Mud zone slows enemy pieces only — store caster so we know whose enemy
    room.effects.push({type:'mud',squares,expiry:now+10000,caster:casterPlayer});

  } else if(spellType==='haste'){
    // Remove cooldown from YOUR piece on the target square
    squares.forEach(({r,c})=>{
      const piece=room.board[r][c];
      if(piece&&piece.player===casterPlayer){
        delete room.cooldowns[`${r},${c}`];
        // Mark this piece as hasted — tracked by position + player
        room.effects.push({
          type:'haste',
          squares:[{r,c}],
          expiry:now+10000,
          caster:casterPlayer,
          player:casterPlayer, // whose piece it is
        });
      }
    });
  }

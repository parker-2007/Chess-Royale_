  ...
  socket.on('disconnect', () => {
    ...
    if(room && !room.isAI && !room.gameOver) {
      ...
      room.disconnectTimer = setTimeout(() => {
        ...
        room.gameOver = true;
        room.winner = disconnectedIdx === 0 ? 2 : 1;
        const eloResult = resolveElo(room, room.winner - 1);
        const state = getPublicState(room);
        io.to(socket.roomCode).emit('state', { ...state, opponentDisconnected: true, eloChange: eloResult ? (disconnectedIdx===0 ? eloResult.p1change : eloResult.p0change) : 0 });
        clearInterval(room.ticker);
        delete rooms[socket.roomCode];
      }, 10000);
      // Tell remaining player opponent disconnected with timer
      socket.to(socket.roomCode).emit('opponentDisconnecting', { seconds: 10 });

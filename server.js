...
function initBoard() {
  ...
  b[1][3]={type:'pawn',player:2}; b[1][5]={type:'pawn',player:2}; b[1][4]={type:'pawn',player:2};
  b[7][4]={type:'king',player:1}; b[7][3]={type:'pawn',player:1}; b[7][5]={type:'pawn',player:1};
  b[6][3]={type:'pawn',player:1}; b[6][5]={type:'pawn',player:1}; b[6][4]={type:'pawn',player:1};
  return b;
}
function moveCost(type,fr,fc,tr,tc) { return PIECE_DATA[type].moveCost(Math.max(Math.abs(tr-fr),Math.abs(tc-fc))); }
...
function createRoom(roomCode) {
  rooms[roomCode] = {
    ...
    elixir:[0,0], hands:[[],[]], queueIdx:[0,0], queue:buildQueue(),
...
function resetRoom(room) {
  ...
  room.queue=buildQueue(); room.promotionQueue=buildPromotionQueue(); room.promotionIdx=0;
  room.cooldowns={}; room.gameOver=false; room.winner=null; room.rematchVotes=new Set();
  room.ticker=null; dealHand(room,0); dealHand(room,1);
}

function dealHand(room,idx) { while(room.hands[idx].length<3) room.hands[idx].push(room.queue[room.queueIdx[idx]++]); }
...
function aiTick(roomCode) {
  ...
  for(let r=0;r<8;r++) for(let c=0;c<8;c++){
    ...
    for(const{row,col}of moves){
      const cost=moveCost(piece.type,r,c,row,col); if(elixir<cost) continue;
  ...
  for(let i=0;i<hand.length;i++){
    const type=hand[i],cost=PIECE_DATA[type].deploy;
...
io.on('connection', (socket) => {
  ...
  socket.on('quickMatch', ({ name, elo }) => {
    ...
    if (matchmakingQueue && matchmakingQueue.socketId !== socket.id) {
      ...
      // Join queue
      matchmakingQueue = { socketId:socket.id, socket, name, elo: elo||1000 };
  ...
  socket.on('forfeit', () => {
    ...
    socket.emit('state',{...state,forfeit:true,eloChange:eloResult?.(p===0?eloResult.p0change:eloResult.p1change)});
    socket.to(socket.roomCode).emit('state',{...state,opponentForfeited:true,eloChange:eloResult?.(p===0?eloResult.p1change:eloResult.p0change)});
  });
  ...
  socket.on('move', ({fromRow,fromCol,toRow,toCol}) => {
    ...
    const cost=moveCost(piece.type,fromRow,fromCol,toRow,toCol); if(room.elixir[p]<cost) return;
    room.elixir[p]-=cost;
    ...
    if(room.board[toRow][toCol]?.type==='king'){
      ...
      const state=getPublicState(room);
      room.socketIds.forEach((id,idx)=>{ const s=io.sockets.sockets.get(id); if(s) s.emit('state',{...state,eloChange:eloResult?.(idx===p?eloResult[`p${idx}change`]:eloResult[`p${idx===0?1:0}change`])||0}); });
      if(room.isAI) io.to(socket.roomCode).emit('state',state);
      return;
    }
    room.board[toRow][toCol]=piece; room.board[fromRow][fromCol]=null;
  ...
  socket.on('deploy', ({cardIdx,row,col}) => {
    const room=rooms[socket.roomCode]; if(!room||room.gameOver) return;
    const p=socket.playerIdx,deployRow=p===0?7:0;
    if(row!==deployRow||room.board[row][col]) return;
    const hand=room.hands[p]; if(cardIdx<0||cardIdx>=hand.length) return;
    const type=hand[cardIdx],cost=PIECE_DATA[type].deploy; if(room.elixir[p]<cost) return;
  ...
  socket.on('disconnect', () => {
    ...
    if(room){ clearInterval(room.ticker); clearTimeout(room.aiTicker); if(!room.isAI) io.to(socket.roomCode).emit('playerLeft'); delete rooms[socket.roomCode]; }

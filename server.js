...
function createRoom(roomCode) {
  rooms[roomCode] = {
    ...
    deployRow:[7,0], // current deploy row for each player
  };
...
function getPublicState(room) {
  ...
  return { board:room.board, elixir:room.elixir, hands:room.hands,
           ...
           timeLeft:Math.max(0,room.timeLeft), deployRow:room.deployRow,
...
function startTicker(roomCode) {
  ...
  room.ticker=setInterval(()=>{
    ...
    // Deploy zone advances every minute
    // At 2:00 left (120s) → row moves up 1, at 1:00 left (60s) → moves up again
    const minutesLeft = Math.ceil(room.timeLeft/60);
    if(minutesLeft < lastMinute) {
      lastMinute = minutesLeft;
      // P1 deploy row moves from 7 toward 0 (up the board)
      // P2 deploy row moves from 0 toward 7 (down the board)
      if(room.deployRow[0] > 4) room.deployRow[0]--;
      if(room.deployRow[1] < 3) room.deployRow[1]++;
      io.to(roomCode).emit('deployZoneChanged', { deployRow: room.deployRow });
    }

    // Game over on timeout — most pieces wins
    if(room.timeLeft<=0){
      room.gameOver=true;
      const p1=countPieces(room.board,1), p2=countPieces(room.board,2);
      if(p1>p2) room.winner=1;
      else if(p2>p1) room.winner=2;
      else room.winner=0; // draw
      const state=getPublicState(room);
      io.to(roomCode).emit('state',{...state, timeout:true, p1pieces:p1, p2pieces:p2});
...
io.on('connection', (socket) => {
  ...
  socket.on('move', ({fromRow,fromCol,toRow,toCol}) => {
    ...
    const cost=moveCost(piece.type,fromRow,fromCol,toRow,toCol,room.timeLeft); if(room.elixir[p]<cost) return;
    room.elixir[p]-=cost;
  ...
  socket.on('deploy', ({cardIdx,row,col}) => {
    ...
    const deployRow=room.deployRow ? room.deployRow[p] : (p===0?7:0);

...
function aiTick(roomCode) {
  ...
  if(!bestMove) return;
  if(bestMove.deploy){const type=hand[bestMove.cardIdx];room.elixir[AI]-=bestMove.cost;room.board[bestMove.row][bestMove.col]={type,player:2};hand.splice(bestMove.cardIdx,1);returnCardToCycle(room,AI,type);dealHand(room,AI);}
